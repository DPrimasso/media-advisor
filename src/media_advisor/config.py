...
# environment/.env — base project config (always loaded first, lower priority)
_base = _env_dir / ".env"
if _base.is_file():
    _env_paths.append(_base)
# environment/.env.local — generated by `npm run env:<provider>:<environment>` (highest priority)
_local = _env_dir / ".env.local"
if _local.is_file():
    _env_paths.append(_local)
...
class Settings(BaseSettings):
    ...
    # Provider URLs per LLM multi-provider
    _PROVIDER_URLS: dict[str, str] = {
        "deepinfra": "https://api.deepinfra.com/v1/openai",
        "groq": "https://api.groq.com/openai/v1",
    }

    def get_provider_chain(self) -> list[tuple[str, str | None]]:
        """Priority: DeepInfra → Groq → OpenAI. Restituisce solo provider con chiave configurata."""
        chain: list[tuple[str, str | None]] = []
        if self.deepinfra_api_key.strip():
            chain.append((self.deepinfra_api_key, self._PROVIDER_URLS["deepinfra"]))
        if self.groq_api_key.strip():
            chain.append((self.groq_api_key, self._PROVIDER_URLS["groq"]))
        if self.openai_api_key.strip():
            chain.append((self.openai_api_key, None))
        return chain

    def get_llm_credentials(self) -> tuple[str, str | None]:
        """Restituisce (api_key, base_url) del provider con priorità più alta configurato."""
        chain = self.get_provider_chain()
        if not chain:
            raise ValueError(
                "Nessun LLM API key configurato (DEEPINFRA_API_KEY, GROQ_API_KEY o OPENAI_API_KEY)"
            )
        return chain[0]

    def get_active_llm_key(self) -> str:
        """Restituisce solo la chiave attiva, per le guard checks. Stringa vuota se nessun provider configurato."""
        chain = self.get_provider_chain()
